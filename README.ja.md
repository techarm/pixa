...
| `paste`              | OS クリップボードの画像をファイル（または標準出力）に保存  |

`compress`, `convert`, `transparent`, `remove-watermark` はファイルとディレクトリの両方を受け付けます。ディレクトリを処理する場合は `-r/--recursive` を付けてください。

すべての画像処理コマンドは、ファイルパスの代わりに `@clipboard`（短縮形 `@clip` / `@c`）を指定することで OS クリップボードから直接画像を読み込めます。詳細は [クリップボード入力](#クリップボード入力) を参照してください。
...
すべての画像処理コマンドは、ファイルパスの代わりに `@clipboard` を入力として受け付け、OS クリップボードから画像を直接読み込めます。短縮形として `@clip` と `@c` も同じ意味で使えます — 好みで選んでください：

```bash
pixa info      @clipboard                         # 寸法・フォーマット・EXIF
pixa compress  @clipboard -o out.webp             # クリップボード画像を圧縮
pixa convert   @clip      out.png                 # @clip は短縮形
pixa favicon   @c         -o ./favicons           # @c が最短
pixa transparent @clipboard -o logo.png --bg '#FF00FF'
pixa split     @clip      -o ./avatars --names a,b,c
pixa rw        @c         -o clean.png
```

クリップボード画像をそのままファイルに保存したい場合（`@clipboard` 入力の逆方向）は、専用の `paste` サブコマンドを使います：

```bash
pixa paste screenshot.png                 # クリップボード → ファイル
pixa paste - --format png | some-tool     # PNG バイト列を標準出力へ
```

**クリップボードに載るもの。** pixa は以下を優先順位で見ます：

1. **ファイル URL** (`public.file-url`) — 例：Finder で画像ファイルを `Cmd+C` した場合。pixa は元ファイルを直接開くので、EXIF・ICC プロファイル・エンコーダー設定など **全てそのまま保持** されます。コピーした JPEG に対して `pixa paste out.jpg` すれば、元ファイルと完全に同一バイトの出力が得られます。
2. **生 PNG バイト列** (`public.png`) — 例：ブラウザで画像を `Cmd+C` した場合。`pixa paste *.png` でバイトそのまま書き出し。他のフォーマットを指定した場合は再エンコード。
3. **デコード済み RGBA ピクセル** — 上記以外（Preview 由来の TIFF、メモリ上のスクショ等）を `arboard` 経由で読み込み。

**プラットフォーム対応。** macOS は 1–3 すべてをネイティブにサポート。Windows と Linux は現状 3（RGBA）のみで、ネイティブなバイト・パススルーはフォローアップ issue [#20](https://github.com/techarm/pixa/issues/20)（Windows）と [#21](https://github.com/techarm/pixa/issues/21)（Linux）で追跡中です。ヘッドレス Linux / SSH セッションでは `clipboard unavailable (no display server…)` と明示的にエラーが出ます（panic はしません）。

**起こりうるエラー。**

- `--recursive` と `@clipboard` は併用できません — クリップボードは画像 1 枚しか持ちません。
- `--output` が通常省略可能なコマンド（保存先を入力の隣に自動生成する）も、`@clipboard` 入力時は `--output` 必須です — 元ファイルの隣に置く、という動作ができないため。
